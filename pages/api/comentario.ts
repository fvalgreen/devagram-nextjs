...
const comentarioEndpoint = async (req: NextApiRequest, res: NextApiResponse<RespostaPadraoMsg>)=>{
    try {
        if(req.method === 'PUT'){ // Verificando o método HTTP do request caso seja algo diferente de PUT retornará que o método não é válido
            ...
            if(!usuarioLogado){ // Caso não retorne nada da busca retorna um erro dizendo que não encontrou o usuário
            ...
            if(!publicacao){ // Caso não retorne nada da busca retorna um erro dizendo que não encontrou a publicação
                return res.status(400).json({erro: 'Publicação não encontrada'});
            }

            if(!req.body || !req.body.comentario || req.body.comentario.length < 2){ // Caso não exista corpo da requisição ou não exista o comentário no corpo ou se o comentário tiver menos de 2 caracteres retorna um erro de comentário inválido
                return res.status(400).json({erro: 'Comentário não é válido'});
            }
            
            const comentario = { // Criamos um objeto com o ID de quem comentou, o nome de quem comentou e o comentário em si
...
export default politicaCORS(validarTokenJWT(conectarMongoDB(comentarioEndpoint))); // Exporta o endpoint de comentário passando pelos middlewares necessários