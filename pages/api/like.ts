...
const likeEndpoint = async (req: NextApiRequest, res: NextApiResponse<RespostaPadraoMsg>) => { // Declaração padrão conforme os outros endpoints
    try {
        if (req.method === 'PUT') { // Verifica se o método HTTP é o PUT caso seja diferente retorna um erro de Método inválido
            // Id da publicação 
            ...
            if (!publicacao) { // Caso não exista a publicação retorna um erro de publicação não encontrada
            ...
            if (!usuario) { // Caso a DB não retorne um usuário um erro de usuário não encontrado é retornado
                return res.status(400).json({ erro: 'Usuário não encontrada' });
            }
            const indexDoUsuarioNoLike = publicacao.likes.findIndex((e: any) => e.toString() === usuario._id.toString()); // Verificamos dentro do Array likes da publicação se já existe o ID do usuário nela. A verificação retornará um index onde o id do usuário está ou retornará -1 caso não exista o id do usuário no Array

            if (indexDoUsuarioNoLike != -1) { // Caso a verificação pelo index retorne algo diferente de -1 significa que o id do usuário já está no Array de likes e o usuário já havia curtido essa publicação, então a ação é para descurtir a publicação
                ...
                return res.status(200).json({ msg: 'Publicação descurtida com sucesso' }); // Retorna uma msg de publicação descurtida
            } else {
    ...
    } catch (e) { // Caso algum erro seja detectado na aplicação é informado no console e retorna uma mensagem de erro
        console.log(e);
